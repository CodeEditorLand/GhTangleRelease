...
async function run() {
	try {
		...
		const tangleExplorer =
			process.env.IOTA_TANGLE_EXPLORER ||
			"https://utils.iota.org/transaction/:hash";
		const provider =
			process.env.IOTA_PROVIDER || "https://nodes.iota.cafe:443";
		let addressIndex = Number.parseInt(process.env.IOTA_ADDRESS_INDEX, 10);
		let depth = Number.parseInt(process.env.IOTA_DEPTH, 10);
		let mwm = Number.parseInt(process.env.IOTA_MWM, 10);
		...
		if (release.data.assets && release.data.assets.length > 0) {
			...
			for (let i = 0; i < release.data.assets.length; i++) {
				...
				const assetHash = await downloadAndHash(
					release.data.assets[i].browser_download_url,
				);
				payload.assets.push({
		...
		console.log(
			`You can view the transaction on the tangle at ${exploreUrl}`,
		);
